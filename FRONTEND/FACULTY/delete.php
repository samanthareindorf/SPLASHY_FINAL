...
if(isset($_POST['DELETEE'])){
    // $_SESSION['deleted']=$_GET['deleteid'];
    ...
    //  echo $event_description;
      ...
      //Delete query to delete event with stated event_id
    $sqlQuery="DELETE FROM events WHERE event_id='$event_id'";
   

    //instance of database
    $database=new Database;

    //set result to the result of the query run
    $result=$database->runQuery($sqlQuery);

    //if query is successful
    if($result){
        echo "<h5 style='color:green'>Successful deletion!</h5>";
                    ...
                    <div class="px-2"><br>
                        <form action="" class="justify-content-center" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="NameEvent" value="<?php echo $eventName?>">  <!--show response in input field-->
                        
                            </div>

                            <div class="form-group">
                            <label>Event Date:</label>
                            <input type="date" name="Day" value="" class="form-control"  value="<?php echo $event_date?> ">    <!--show response in input field-->

                            <div class="form-group">
                            <label>Event Start Time:</label>
                            <input type="time" name="startTime" class="form-control" value="<?php echo $event_time?>">    <!--show response in input field-->
                            </div>

                            <div class="form-group">

                            <label>Event Details:</label> <br>
                           <input type="textarea" name="Description" class="form-control"  value="<?php echo $event_description?> ">   <!--show response in input field-->

                            </div>

                            </div>
                            <button type="submit" name="DELETEE" class="btn btn-danger"><i class='fas fa-trash'>Delete Event</i></button>
                            <span><button onclick="openViewEventsPage()" class="btn btn-secondary"><i class="fa fa-eye">View Current Events</i></button></span>
                        </form>
                    </div>
                </div>
            </div>
